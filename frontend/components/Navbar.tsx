...
const Navbar: React.FC<NavbarProps> = ({ currentUser, onLogout, onLogin }) => {
        ...
        <div className="flex justify-between h-16">
          <div className="flex items-center">
            <Link to="/" className="flex-shrink-0 flex items-center">
              <span className="text-2xl font-bold text-primary-600">Sewa<span className="text-secondary-900">Sanjal</span></span>
            </Link>
            <div className="hidden sm:ml-8 sm:flex sm:space-x-4">
              <Link to="/" className={navLinkClass('/')}>Home</Link>
              <Link to="/explore" className={navLinkClass('/explore')}>Explore Services</Link>
              {currentUser && (
              ...
              <div className="flex items-center space-x-4">
                <div className="flex items-center space-x-2">
                   {currentUser.avatarUrl ? (
                     <img src={currentUser.avatarUrl} alt={currentUser.name} className="h-8 w-8 rounded-full border border-gray-200" />
                   ) : (
            ...
            <div className="mt-4 pt-4 border-t border-gray-200">
              {currentUser ? (
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => { onLogout(); setIsOpen(false); }} className="text-red-600 font-medium">Logout</button>
                </div>
              ) : (
                <button 
