...
const ProviderDetails: React.FC<ProviderDetailsProps> = ({ currentUser }) => {
  ...
  const [activeTab, setActiveTab] = useState<"services" | "reviews" | "about">(
    "services",
  );
            ...
            <div className="flex flex-col md:flex-row items-start gap-6">
              ...
              <div className="mt-2">
                <div className="flex items-center gap-2">
                  ...
                  {provider?.isVerified && (
                    <BadgeCheck className="text-blue-500 w-6 h-6 fill-blue-50" />
                  )}
                </div>
                ...
                <div className="flex items-center gap-4 mt-4">
                  <div className="flex items-center bg-amber-50 px-3 py-1.5 rounded-full text-amber-700 font-bold text-sm border border-amber-100">
                    <Star
                      size={16}
                      className="fill-amber-400 text-amber-400 mr-1.5"
                    />
                    {provider?.rating}{" "}
                    <span className="text-amber-600 ml-1 font-normal">
                      ({provider?.reviewCount} Reviews)
                    </span>
                  </div>
                  <span className="text-gray-300">|</span>
                  <span className="text-gray-600 text-sm font-medium bg-gray-100 px-3 py-1 rounded-full">
                    {provider?.categories[0]?.name} Specialist
                  </span>
                </div>
              </div>
            </div>
          ...
          {activeTab === "services" && (
            <div className="space-y-4 animate-fadeIn">
              ...
              {provider.services.map((service) => (
                <div
                  key={service.id}
                  className="bg-white border border-gray-200 rounded-xl p-5 hover:border-primary-300 hover:shadow-md transition-all group flex flex-col sm:flex-row justify-between sm:items-center gap-4"
                >
                  <div>
                    <h3 className="font-bold text-lg text-gray-900 group-hover:text-primary-700">
                      {service.title}
                    </h3>
                    ...
                    <div className="flex items-center mt-3 text-xs text-gray-500 bg-gray-50 inline-flex px-2 py-1 rounded">
                      <Clock size={12} className="mr-1" /> {service.durationMin}{" "}
                      mins duration
                    </div>
                  </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 sticky top-24">
            ...
            <ul className="space-y-4 text-sm text-gray-600">
              <li className="flex items-start">
                <div className="bg-green-100 p-1 rounded-full mr-3 text-green-600">
                  <BadgeCheck size={16} />
                </div>
                <span>
                  <strong>Verified Background:</strong> We have checked their
                  identity and criminal record.
                </span>
              </li>
              <li className="flex items-start">
                <div className="bg-blue-100 p-1 rounded-full mr-3 text-blue-600">
                  <ThumbsUp size={16} />
                </div>
                <span>
                  <strong>{provider.reviewCount}+ Jobs Completed:</strong>{" "}
                  Highly experienced in their field.
                </span>
              </li>
              <li className="flex items-start">
                <div className="bg-purple-100 p-1 rounded-full mr-3 text-purple-600">
                  <Star size={16} />
                </div>
                <span>
                  <strong>High Rating:</strong> Maintained a {provider.rating}{" "}
                  star average.
                </span>
              </li>
            </ul>
            ...
            <div className="mt-6 pt-6 border-t border-gray-100">
              ...
              <button className="w-full text-center text-primary-600 text-sm font-medium hover:underline">
                Report this profile
              </button>
            </div>
          </div>
