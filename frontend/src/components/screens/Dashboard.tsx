...
import { Calendar, Clock, MapPin, DollarSign, CheckCircle, XCircle, AlertCircle, Plus, Edit2, Trash2, TrendingUp } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ user }) => {
  ...
  const [activeTab, setActiveTab] = useState<'UPCOMING' | 'HISTORY' | 'SERVICES'>('UPCOMING');

  useEffect(() => {
    const loadData = async () => {
      setIsLoading(true);
      const data = await MockService.getBookingsForUser(user.id, user.role === Role.CUSTOMER ? 'CUSTOMER' : 'PROVIDER');
      setBookings(data);
      
      if (user.role === Role.PROVIDER) {
         // Assuming we can get provider ID from somewhere or search it, 
         // for mock we know user u2 is p1
         const providers = await MockService.getProviders();
         const myProvider = providers.find(p => p.userId === user.id);
         if (myProvider) {
            const services = await MockService.getProviderServices(myProvider.id);
  ...
  const upcomingBookings = bookings.filter(b => [BookingStatus.PENDING, BookingStatus.ACCEPTED, BookingStatus.IN_PROGRESS].includes(b.status));
  const pastBookings = bookings.filter(b => [BookingStatus.COMPLETED, BookingStatus.CANCELLED, BookingStatus.REJECTED].includes(b.status));
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 bg-gray-50 min-h-screen">
      <div className="mb-8 flex justify-between items-end">
        <div>
           ...
           <p className="text-gray-500 mt-1">Welcome back, <span className="font-semibold text-gray-800">{user.name}</span></p>
        </div>
        {user.role === Role.PROVIDER && (
           <span className="px-3 py-1 bg-primary-100 text-primary-700 rounded-full text-sm font-medium">Provider Account</span>
        )}
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 lg:col-span-2">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-gray-900 flex items-center"><TrendingUp size={20} className="mr-2 text-primary-600"/> Weekly Performance</h3>
            ...
            <div className="h-64 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={analyticsData}>
                  ...
                  <YAxis axisLine={false} tickLine={false} tick={{fill: '#64748b'}} tickFormatter={(value) => `NPR ${value}`} />
                  <Tooltip 
                    cursor={{fill: '#f1f5f9'}}
                  ...
                  <Bar dataKey="earnings" fill="#0d9488" radius={[4, 4, 0, 0]}>
                    {analyticsData.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={index === 5 ? '#0f766e' : '#14b8a6'} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900">NPR 125,000</p>
                </div>
              </div>
            </div>
             <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
      ...
      <div className="bg-white shadow-sm rounded-lg border border-gray-200 mb-6 overflow-hidden">
        <nav className="flex divide-x divide-gray-200">
          ...
          >
            {user.role === Role.PROVIDER ? 'Incoming & Active' : 'Upcoming Bookings'}
          </button>
          <button
      ...
      {isLoading ? (
           ...
           {providerServices.length > 0 ? (
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               {providerServices.map(service => (
                 <div key={service.id} className="bg-white p-5 rounded-lg border border-gray-200 shadow-sm flex justify-between items-start">
                    <div>
                      <h4 className="font-bold text-gray-900">{service.title}</h4>
                      <p className="text-sm text-gray-500 mt-1">{service.description}</p>
                      <div className="flex items-center gap-4 mt-3 text-sm">
                         <span className="font-semibold text-primary-700">NPR {service.price}</span>
                         <span className="text-gray-400">|</span>
                         <span className="text-gray-600">{service.durationMin} mins</span>
                      </div>
                    </div>
                    <div className="flex gap-2">
                       <button className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded"><Edit2 size={16} /></button>
        ...
        <div className="text-center py-12 bg-white rounded-lg border border-dashed border-gray-300">
          ...
          <p className="mt-1 text-sm text-gray-500">
             {activeTab === 'UPCOMING' ? "You don't have any upcoming bookings." : "You haven't completed any bookings yet."}
          </p>
        </div>
      ) : (
                ...
                <div className="px-4 py-5 sm:px-6">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                       <div className="flex-shrink-0 relative">
                         <img 
                           ...
                           src={user.role === Role.CUSTOMER ? booking.provider?.user.avatarUrl : booking.customer?.avatarUrl} 
                           alt="" 
                         />
                         <div className={`absolute -bottom-1 -right-1 h-4 w-4 rounded-full border-2 border-white ${booking.status === BookingStatus.IN_PROGRESS ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                       </div>
                       <div className="ml-4">
                         <p className="text-sm font-bold text-primary-900">
                           {user.role === Role.CUSTOMER ? booking.provider?.user.name : booking.customer?.name}
                         </p>
                         <p className="text-sm text-gray-500 font-medium">{booking.service?.title}</p>
                       </div>
                    </div>
                    <div className="flex flex-col items-end">
                      ...
                      <p className="mt-1 text-sm text-gray-900 font-bold">NPR {booking.price}</p>
                    </div>
                  </div>
                  <div className="mt-4 sm:flex sm:justify-between items-center">
                    <div className="sm:flex gap-6">
                      <p className="flex items-center text-sm text-gray-500">
                        ...
                        {new Date(booking.scheduledAt).toLocaleDateString(undefined, { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' })}
                      </p>
                      <p className="flex items-center text-sm text-gray-500 mt-2 sm:mt-0">
                        <Clock size={16} className="flex-shrink-0 mr-1.5 text-gray-400" />
                        {new Date(booking.scheduledAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                      </p>
                    </div>
                    
                    {/* Action Buttons for Provider */}
                    {user.role === Role.PROVIDER && booking.status === BookingStatus.PENDING && (
                      <div className="mt-4 sm:mt-0 flex items-center space-x-3">
                        ...
                        >
                          <CheckCircle size={14} className="mr-2"/> Accept Request
                        </button>
                        <button 
                    ...
                    {user.role === Role.CUSTOMER && booking.status === BookingStatus.PENDING && (
